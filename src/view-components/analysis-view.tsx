	...
	render() {
		return (
			<main>
				{this.showCross ? (
					<div>
						<div className="cursor-line vertical" style={{ left: this.mouseCoords[0] + 3 }} />
						<div className="cursor-line horizontal" style={{ top: this.mouseCoords[1] + 3 }} />
					</div>
				) : (
					''
				)}
							...
							<div key={line.id}>
								<div
									...
									style={{
										...
										borderColor: this.markLines.indexOf(line.id) !== -1 ? 'gold' : '#212121',
										lineHeight: '1px',
										position: 'absolute',
										left: line.left + 'px',
										top: line.top + 'px',
								...
								<div
									...
									style={{
										display: this.showAdditionalLines ? '' : 'none',
										padding: 0,
										margin: 0,
										height: 1 + 'px',
										borderBottom: '1px dashed #e3e3e3',
										borderColor: this.markLines.indexOf(line.id) !== -1 ? 'gold' : '#e3e3e3',
										lineHeight: '1px',
										position: 'absolute',
										left: (line.x || { x_left: 0 }).x_left + 'px',
										top: (line.x || { x_top: 0 }).x_top + 'px',
										width: (line.x || { x_distance: 0 }).x_distance + 'px',
										transform: 'rotate(' + (line.x || { x_angle: 0 }).x_angle + 'deg)'
									}}
								/>
							</div>
						))}
				...
				<div className="controls">
					<div className="control-button">
						<span
							...
							onClick={() => (this.showAdditionalLines = this.showAdditionalLines ? false : true)}
						>
							⇢
						</span>
					</div>
					<div className="control-button">
						<span
							className="control-symbol"
							onClick={() => (this.showPointNames = this.showPointNames ? false : true)}
						>
							♇
						</span>
					</div>
					<div className="control-button">
						<span
							className="control-symbol"
							onClick={() => (this.showCross = this.showCross ? false : true)}
						>
							✛
						</span>
					</div>
					<div className="control-button">
						<span
							className="control-symbol"
							onClick={() => (this.showControl = this.showControl === 'brightness' ? '' : 'brightness')}
						>
							☀
						</span>
						<div
							...
							<table>
								...
								<tbody>
									{data.analysisMethod.anglesValues
										.concat(data.analysisMethod.DistanceValues)
										.map((param) => {
